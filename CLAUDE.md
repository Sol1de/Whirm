...
- **Node.js** ≥ 18 + **pnpm**
- **Rust** ≥ 1.85 (`rustup` recommended) — required only for `pnpm tauri dev` / `pnpm tauri build`
- Tauri system dependencies: see [tauri.app/start/prerequisites](https://tauri.app/start/prerequisites/)

## Commands

```bash
pnpm install      # Install dependencies (run after clone or lockfile changes)
...
└── backend/           — native desktop app (Rust + Tauri 2)
    └── src/
        ...
        ├── commands/  — Tauri command handlers (proxy, settings, session, notification)
        ├── crypto.rs  — AES-256-GCM encryption
        ├── net/       — local relay, route engine, transports, system proxy
...
- **`proxy.rs`** — `connect_proxy`, `disconnect_proxy`, `test_proxy`, `test_proxy_by_id`, `get_throughput`, `get_proxies`, `add_proxy`, `update_proxy`, `delete_proxy`
...
**Password encryption**: proxy passwords are encrypted at rest via AES-256-GCM with a machine-specific key derived using HKDF-SHA256 (`machine-uid` crate, salt `"whirm-field-v1"`). Implementation in `backend/src/crypto.rs`. `add_proxy`/`update_proxy` encrypt before DB write; `connect_proxy` decrypts before use. Never compare or forward the raw DB `password` field — it is ciphertext. The key is machine-bound, so OS reinstall / hardware changes can make stored passwords undecryptable — `connect_proxy`/`test_proxy_by_id` map the `"Decryption failed"` error to a user-facing "re-enter the password" message (`map_decrypt_err`).
